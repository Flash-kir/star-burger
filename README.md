...
В [кабинете разработчика](https://developer.tech.yandex.ru/) в API Яндекс-геокодера получите ключ. В созданном файле `.env` в каталоге `star_burger/` добавьте `API_YANDEX_KEY`:
```sh
API_YANDEX_KEY=ee3c2e2f-c5d5-4...
```
...
для сообщения в `rollbar` о деплое и корректной работы скрипта создайте переменную `$rollbar_token` 
и присвойте ей значение токена `rollbar`:
```sh
echo $rollbar_token
rollbar_token=b857e5426724409...78fd7b62f1776b
```
